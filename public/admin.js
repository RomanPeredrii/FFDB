...
document.querySelectorAll(".deleteUserButton").forEach((node) => {
  node.addEventListener("click", async () => {
    try {
      await fetch(
        `/admin/${node.parentNode.textContent
          .trim()
          .split("\n")
          [
...
document.querySelectorAll(".editUserButton").forEach((node) => {
  node.addEventListener("click", async () => {
    try {
      ...
      document.querySelectorAll("#change-existing input").forEach((node) => {
        body.user[node.name] ? (node.value = body.user[node.name]) : null;
      });
    } catch (error) {
